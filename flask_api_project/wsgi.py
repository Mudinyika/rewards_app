...
from franchise_routes import franchise_bp
import json
import re
...
from models import db, Operator, Admin
...
SHOP_ID = config[env].SHOP_ID  # ✅ Access the SHOP_ID from the correct config class
...
# Register the Franchise Blueprint
app.register_blueprint(franchise_bp, url_prefix="/api")
...
# ✅ Till Operator Login
@app.route("/api/franchise/login", methods=["POST"])
def franchise_login():
    data = request.get_json()
    phone_number = data.get("phone_number")  # 🔹 Use phone number instead of username
    password = data.get("password")

    # 🔹 Check if the franchise manager exists
    franchise_manager = Admin.query.filter_by(admin_phone_number=phone_number, role="franchise_manager").first()
    if not franchise_manager or not check_password_hash(franchise_manager.password_hash, password):
        return jsonify({"error": "Invalid credentials"}), 401

    # 🔹 Generate JWT Token
    access_token = create_access_token(
    identity=str(franchise_manager.id),
    additional_claims={"role": franchise_manager.role}
)
    ...
    return jsonify({
        ...
        "franchise_id": franchise_manager.franchise_id,
        "role": franchise_manager.role,
...
def allocate_points():
    ...
    till_number = str(data.get("till_number")) if data.get("till_number") else None # ✅ Only required for Admins
